...
### 2. 设计出一个可以评价参数效果的惩罚函数。

给定一个参数向量以后，我们需要对这个参数向量的分类效果进行评价。具体的想法就是，设计惩罚函数r(x,a)，r(x,a)这个函数满足如下性质：
...
![equation](http://latex.codecogs.com/gif.latex?r(x,a)=\frac{e^{a\cdot\widetilde{x}}}{1+a\cdot\widetilde{x}})。
